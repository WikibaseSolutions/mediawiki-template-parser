...
{
    /**
     * @var int The current anonymous argument pointer
     */
    private int $anonymousArgumentPointer;

    /**
     * This function parses an article. It looks for all the templates on the article, then returns an array
     * containing the parameters of those templates.
     *
     * For example:
     *
     * [
    ...
    {
        $templateSources = $this->findTemplates($articleSource);
    ...
    protected function tokenizeTemplate(string $template): array
    {
        ...
        foreach ($template as $index => $char) {
            if ($char === "{" && $template[$index + 1] === "{") { // Check if a template starts
                $nestingDepth++;
            } else if ($nestingDepth > 0 && $char === "}" && $template[$index + 1] === "}") {
                // Check if a template ends
                $nestingDepth--;
            } else if ($nestingDepth === 0 && $char === "|") {
                $arguments[] = $buffer;
                $buffer = '';
    ...
    protected function parseArgument(string $argument): array
    {
        ...
        if (count($parts) === 1) {
            // Anonymous argument
            return [strval($this->anonymousArgumentPointer++), trim($parts[0])];
        }

        // Named argument
        $argument_name = trim(array_shift($parts));
        $argument_value = trim(implode("=", $parts));
