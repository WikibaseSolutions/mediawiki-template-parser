...
class RecursiveParser {
	 ...
	 */
	public function handlePipes( string $source, bool $reverse = false ) {
		if ( $reverse ) {
			$find = '%%SWPIPE%%';
			$replace = '|';
	...
	private function splitArguments( string $template ): array {
		...
		while ( $idx < $numChars ) {
			...
			} elseif ( substr( $template,
					$idx,
					3 ) === self::PARAM_START ) {
				// We found a parameter on which we should not split
				$argument .= $this->match( $template,
					$numChars,
					$idx,
					self::PARAM_START,
					self::PARAM_END );
		...
		$arguments[] = $this->handlePipes( $argument, true );
